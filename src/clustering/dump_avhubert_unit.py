...
logger = logging.getLogger("dump_avhubert_unit")
...
class HubertFeatureReader(object):
    ...
    def load_feature(self, mix_name, ref_len=None):
        ...
        audio_npy = audio_fn.replace('.mp4','.npy')
        ##import pdb;pdb.set_trace()
    ...
    def get_units(self, path, ref_len=None, modality='v'):
        ...
        with torch.no_grad():
            ...
            if modality == 'v':
                audio_feats = 0 * audio_feats
            elif modality == 'a':
                video_feats = 0 * video_feats
            ...
            units = self.model.extract_units(
                source=source,
                padding_mask=None,
                mask=False,
...
def dump_units(
    ...
    feat_path = f"{feat_dir}/{split}_{rank}_{nshard}.units"
    ...
    with open(leng_path, "w") as leng_f:
        with open(feat_path, 'w') as units_f:
            for path, nsample in tqdm.tqdm(iterator, total=num):
                units = reader.get_units(path, nsample, kwargs['modality'])
                units = units.cpu()
                str_units = [str(x.item()) for x in units[0]]
                units_f.write(' '.join(str_units) + '\n')
                leng_f.write(f"{len(units)}\n")
