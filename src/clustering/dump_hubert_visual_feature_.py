...
class HubertFeatureReader(object):
    def __init__(self, ckpt_path, layer, image_aug, max_chunk=1600000, custom_utils=None):
        (
            model,
            cfg,
            task,
...
def dump_feature(
        tsv_dir, split, ckpt_path, layer, image_aug, nshard, rank, feat_dir, max_chunk, custom_utils=None, **kwargs
):
    reader = HubertFeatureReader(ckpt_path, layer, image_aug, max_chunk, custom_utils=custom_utils)
    generator, num = get_path_iterator(f"{tsv_dir}/{split}.tsv", nshard, rank)
    iterator = generator()

    for path, nsample in tqdm.tqdm(iterator, total=num):
        vid_pth = path[0]
        output_pth = vid_pth.replace('/mnt/ssd/Dataset/','/mnt/ssd/jh/Exp/acl24/Dataset_feat/')

        os.makedirs(os.path.dirname(output_pth), exist_ok=True)
        feat = reader.get_feats(path, nsample)
        #import pdb;pdb.set_trace()
        np.save(output_pth[:-4], feat.cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_aug", type=bool)
